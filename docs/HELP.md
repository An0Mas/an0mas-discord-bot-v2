...
- `📢メンション`：参加者全員にメンションを送ります（作成者のみ）

### 権限

- `締切/再開` と `＋/－` と `編集` と `📢メンション` は **募集作成者のみ** 実行可能です。

### 注意

- `slots/title/body` を一部だけ指定した場合はエラーになります（no-op：何も投稿されません）。
- 締切中は `参加/参加取消/＋/－` が無効になります。
...
- **ロール変更**：別ロールのボタンを押すと自動でロールが切り替わります（変更先に空きがある場合）。
- **ロール別人数制限**：各ロールで定員に達すると参加できなくなります。

### 権限

- `締切/再開` と `編集` と `📢メンション` は **募集作成者のみ** 実行可能です。

### 例

- `/bosyu-bpsr` → モーダルで タイトル:`真竜周回`, 内容:`21:00〜`, タンク:`1`, アタッカー:`2`, ヒーラー:`1`
...
## 既知の制約（2026-02-14 監査時点）

### /remind

- 送信失敗時もリマインダーを削除する実装のため、一時障害で通知が失われる可能性があります。

### /mention-reactors

- リアクションユーザー取得は100件上限です。
- 429（Rate Limit）時の再試行は未対応です。

### /bosyu / /bosyu-bpsr

- 同時操作時の競合対策は改善予定です。
- 参加者多数時のメンション分割送信は改善予定です（`/bosyu-bpsr` は大規模運用要件が確定次第）。
