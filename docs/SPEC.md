...
## 9. 監査起点の改善タスク（2026-02-14）

`docs/research/codex-audit/2026-02-14/improvement-tracker.md` の優先タスクに対応する仕様メモ。
以下は「現状実装の制約」と「改善予定」を明示したもの。

| #   | タスク                                   | 現状                                             | 改善予定                                              |
| --- | ---------------------------------------- | ------------------------------------------------ | ----------------------------------------------------- |
| 1   | 権限設定INSERTの例外分類導入             | 重複制約違反とDB障害が同じ扱いになる可能性がある | `SQLITE_CONSTRAINT` のみ重複扱いに分離                |
| 2   | 募集メンション分割送信対応（bosyu/bpsr） | 参加者多数時に文字数上限で失敗する可能性         | メンション分割送信を導入（`bosyu-bpsr` は要件確定後） |
| 3   | Precondition拒否通知のレベル分離         | 想定内拒否が監視ノイズ化する可能性               | 想定内拒否と異常系を通知レベルで分離                  |
| 4   | リマインダー失敗時の再試行化             | 送信失敗時も削除され通知が消失する可能性         | 成功時のみ削除し、失敗時は再試行                      |
| 5   | mention-reactors 全件取得 + 429再試行    | リアクション取得100件上限、429再試行なし         | ページング取得と再試行導入                            |
| 6   | 募集ボタン競合対策（再取得再計算）       | 同時操作でロストアップデート可能性               | 更新前再取得 + 再計算                                 |
| 7   | catch時の障害通知経路明示化              | 一部catchが通知経路不明瞭                        | catch経路の通知ルールを明示                           |
| 8   | 募集メンション系のTextBasedChannel対応   | Thread運用で型安全性に改善余地                   | `TextBasedChannel` ベースへ統一                       |
| 9   | DBスキーマバージョン管理導入             | `schema_version` 運用未実装                      | 起動時マイグレーション基盤を導入                      |
| 10  | DX品質ゲート（format/CI）整備            | CIワークフロー未整備                             | 最小CI（`verify` + `format:check`）導入               |
