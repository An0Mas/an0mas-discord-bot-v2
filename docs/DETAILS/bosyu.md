...
## 11. 既知の制約と改善予定（2026-02-14）

- メンション送信:
  - 現状: 参加者が多いと1メッセージの文字数上限（2000文字）に達し、送信失敗する可能性がある。
  - 対応方針: メンション本文の分割送信を導入する（大規模運用要件が確定次第）。
- 同時操作:
  - 現状: 同時クリック時に後勝ち上書きでロストアップデートが起こる可能性がある。
  - 対応方針: 更新前に最新メッセージを再取得して再計算する。
- チャンネル型:
  - 現状: メンション系で `TextChannel` 前提が残っており、Threadでの扱いに改善余地がある。
  - 対応方針: `TextBasedChannel` ベースへ統一する。
