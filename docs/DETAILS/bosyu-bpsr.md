...
## 12. 既知の制約と改善予定（2026-02-14）

- メンション送信:
  - 現状: 参加者が多い場合、1メッセージの文字数上限（2000文字）で送信失敗する可能性がある。
  - 対応方針: 参加者メンションを分割送信する。
- 同時操作:
  - 現状: 同時クリック時に更新競合が発生し、参加状態が上書きされる可能性がある。
  - 対応方針: 更新直前に最新メッセージを再取得して再計算する。
- チャンネル型:
  - 現状: メンション系処理でThread運用に対する型安全性に改善余地がある。
