...
1. `main` ではなく作業ブランチ（例: `agent/<short-task>`）で作業する
2. コード変更時は `pnpm lint:fix` → `pnpm format` → `pnpm verify` を実行する
3. PRは固定書式を使い、AIは `push` 後にPRタイトル/本文ドラフトを提案する
...
- **AIが関与したコード変更は必ずPR経由で取り込む**

### PR前チェック（最小）

1. 変更ファイルと影響範囲が意図通りか確認する
2. コード変更がある場合は `pnpm lint:fix` → `pnpm format` → `pnpm verify` を実行する
3. PR本文に最低限 `目的 / 変更点 / 検証結果` を記載する

> 補足: ドキュメントのみの変更は `pnpm verify` を省略可。ただし省略した旨をPR本文に明記すること。

### PR書式（固定）

- タイトル形式: `<type>: <変更の要約>`
  - 例: `docs: AGENTS.mdにPR運用ルールを追記`
- AIは、`push` 後に必ずPRタイトルと本文ドラフトを提案する。
- 本文テンプレートは以下を使用する。
...
## 検証結果
- `pnpm verify`: 成功/失敗/未実施
- `pnpm format:check`: 成功/失敗/未実施

## 未実施項目
- 

## 確認事項
- [ ] base / compare ブランチが正しい
- [ ] 検証結果（`pnpm verify` 等）を本文に記載した
- [ ] 破壊的変更の有無を明記した
```
...
- `pnpm lint:fix` → `pnpm format` → `pnpm verify`
