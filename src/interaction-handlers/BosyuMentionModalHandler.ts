/**
 * /bosyu メンションモーダルハンドラ — Sapphire InteractionHandler 形式
 */
...
import {
  ...
  parseBosyuEmbed,
  parseBosyuMentionModalSubmission,
  parseBosyuMentionModalTarget,
} from '../lib/bosyu-utils.js';

export class BosyuMentionModalHandler extends InteractionHandler {
  public constructor(
  ...
  public override parse(interaction: ModalSubmitInteraction) {
    const parsed = parseBosyuMentionModalTarget(interaction.customId);
    if (!parsed) {
      return this.none();
    }
  ...
  public override async run(
    ...
    // カスタムメッセージを取得
    const customMessage = parseBosyuMentionModalSubmission(interaction);
    if (!customMessage) {
      await interaction.reply({
    ...
    // メンション送信
    const mentionMessage = buildBosyuMentionMessage(state.members, customMessage);
