...
export function buildHelpDetail(input: HelpDetailBuildInput) {
  const embed = new EmbedBuilder().setTitle(input.entry.name).setDescription(input.entry.detail);
...
function buildNumberRows(entries: HelpEntry[], startIndex: number, userId: string, page: number) {
