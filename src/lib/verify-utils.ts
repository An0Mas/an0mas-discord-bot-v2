...
export function buildVerifyEmbed(setting: VerifySetting): EmbedBuilder {
  const embed = new EmbedBuilder().setTitle(setting.title ?? '🔐 認証').setColor(0x5865f2); // Discord Blurple
...
export function buildVerifyModal(messageId: string): ModalBuilder {
  const modal = new ModalBuilder().setCustomId(`verify-modal:${messageId}`).setTitle('認証');
...
// ボタンカスタムIDをパース（認証ボタン）
export function parseVerifyButtonId(
  customId: string,
): { messageId: string; ownerId: string } | null {
...
// 編集ボタンカスタムIDをパース
export function parseVerifyEditButtonId(
  customId: string,
): { messageId: string; ownerId: string } | null {
...
// 削除ボタンカスタムIDをパース
export function parseVerifyDeleteButtonId(
  customId: string,
): { messageId: string; ownerId: string } | null {
