...
// モーダル入力をパース
export function parseRemindModalSubmission(
...
// リマインダー一覧Embedを生成
export function buildRemindListEmbed(
  reminders: Reminder[],
  userId: string,
): {
  embed: EmbedBuilder;
...
// ボタンのcustomIdをパース
export function parseRemindListCustomId(
  customId: string,
): { userId: string; reminderId: number } | null {
