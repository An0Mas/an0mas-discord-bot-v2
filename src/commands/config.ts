...
/**
 * /config permissions — コマンドの権限設定を表示
 */
async function handleConfigPermissions(interaction: ChatInputCommandInteraction): Promise<void> {
    const guildId = interaction.guildId;
    if (!guildId) {
        await interaction.reply({
...
export async function handleConfigCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (subcommand === "permissions") {
        await handleConfigPermissions(interaction);
        return;
    }
