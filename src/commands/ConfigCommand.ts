...
export class ConfigCommand extends Command {
  ...
  public override registerApplicationCommands(registry: Command.Registry) {
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addSubcommand((sub) => sub.setName('show').setDescription('現在の設定を表示'))
        .addSubcommand((sub) =>
          sub
            .setName('permissions')
            .setDescription('コマンドの権限設定を表示')
            .addStringOption((opt) =>
              opt.setName('command').setDescription('コマンド名（省略で一覧）').setRequired(false),
            ),
        ),
    );
  }
  ...
  private async handlePermissions(interaction: Command.ChatInputCommandInteraction) {
    ...
    if (commandName) {
      ...
      const userList =
        allowedUsers.length > 0 ? allowedUsers.map((id) => `<@${id}>`).join('\n') : '（なし）';
      const roleList =
        allowedRoles.length > 0 ? allowedRoles.map((id) => `<@&${id}>`).join('\n') : '（なし）';
      ...
      const { COMMANDS, isOwnerOnlyCommand, isRestrictedCommand } =
        await import('../command-config.js');
