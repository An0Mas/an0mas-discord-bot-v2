    ...
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addNumberOption((option) =>
          option.setName('slots').setDescription('募集人数（1〜99）').setRequired(false),
        )
        .addStringOption((option) =>
          option.setName('title').setDescription('タイトル（1〜100文字）').setRequired(false),
        )
        .addStringOption((option) =>
          option.setName('body').setDescription('本文（1〜1000文字）').setRequired(false),
        ),
    );
  }
