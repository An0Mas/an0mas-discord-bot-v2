...
export function initDatabase() {
  ...
  // メタテーブル
  database.prepare('CREATE TABLE IF NOT EXISTS meta (key TEXT PRIMARY KEY, value TEXT)').run();
  ...
  database
    .prepare('CREATE INDEX IF NOT EXISTS idx_reminders_notify_at ON reminders(notify_at)')
    .run();
  ...
  // インデックス作成
  database
    .prepare('CREATE INDEX IF NOT EXISTS idx_verify_settings_guild ON verify_settings(guild_id)')
    .run();
}
...
export function isGuildEnabled(guildId: string): boolean {
  const database = getDb();
  const row = database
    .prepare('SELECT enabled FROM guild_config WHERE guild_id = ?')
...
export function getAllEnabledGuilds(): GuildConfig[] {
  const database = getDb();
  return database.prepare('SELECT * FROM guild_config WHERE enabled = 1').all() as GuildConfig[];
}
...
export function removeAllowedUser(guildId: string, command: string, userId: string): boolean {
  const database = getDb();
  const result = database
    .prepare('DELETE FROM allowed_users WHERE guild_id = ? AND command = ? AND user_id = ?')
    .run(guildId, command, userId);
  return result.changes > 0;
}
...
export function getAllowedUsers(guildId: string, command: string): string[] {
  const database = getDb();
  const rows = database
    .prepare('SELECT user_id FROM allowed_users WHERE guild_id = ? AND command = ?')
...
export function removeAllowedRole(guildId: string, command: string, roleId: string): boolean {
  const database = getDb();
  const result = database
    .prepare('DELETE FROM allowed_roles WHERE guild_id = ? AND command = ? AND role_id = ?')
    .run(guildId, command, roleId);
  return result.changes > 0;
}
...
export function getAllowedRoles(guildId: string, command: string): string[] {
  const database = getDb();
  const rows = database
    .prepare('SELECT role_id FROM allowed_roles WHERE guild_id = ? AND command = ?')
...
export function getVerifySetting(messageId: string): VerifySetting | null {
  const database = getDb();
  const row = database
    .prepare('SELECT * FROM verify_settings WHERE message_id = ?')
...
export function updateVerifySetting(
  ...
  const result = database
    .prepare(`UPDATE verify_settings SET ${sets.join(', ')} WHERE message_id = ?`)
    .run(...values);
  return result.changes > 0;
}
...
export function deleteVerifySetting(messageId: string): boolean {
  const database = getDb();
  const result = database
    .prepare('DELETE FROM verify_settings WHERE message_id = ?')
    .run(messageId);
  return result.changes > 0;
}
